// Based on https://github.com/blackinitial/phoenix-config/blob/639bd6b09f490adbde0eb06760a8f61d990c8645/.phoenix.js
...
class ChainWindow {
  ...
  magnet_to(window) {
    // Phoenix.log("Parent:" + JSON.stringify(this.parent));
    // Phoenix.log("Frame :" + JSON.stringify(this.frame));
    
    const new_parent = {...this.parent};
    new_parent.x = window.frame.x + window.frame.width;
    // Phoenix.log("New parent:");
    // Phoenix.log(JSON.stringify(new_parent));

    this.parent = new_parent;
    // TODO(bzz):
    //  move left, without adjusting Ys
    //  fit last window to the screen (no overflow)
    this.to(NW);
    return this;
  }
...
// Magnete To window
Window.prototype.magnet_to = function (win) {
    // Phoenix.log("magneting to:" + win.hash())
    const window = this.chain();
    window.magnet_to(win.chain()).set();
  };
...
Key.on('return', CONTROL_SHIFT, () => {
  const focused_window = Window.focused();
  // Phoenix.log("Window focused:", focused_window.hash(), focused_window.frame().x)
  if (!focused_window) {
    return
  }
  
  // Phoenix.log(focused_window.hash())
  // const wins = window.app().windows({visible: true})
  // wins.map(n => Phoenix.log(n.hash()))

  const wins = App.focused().windows({visible: true})
  // Phoenix.log("Windows found ", wins.length)
  const rest = _.filter(wins, function(w) {
      return w.hash() != focused_window.hash()
  })

  // rest.forEach(w => Phoenix.log("\t" + w.hash() + " " + w.frame().x));
  ...
  const sorted_left = _
    ...
    .sortBy(w => w.frame().x)
    .filter(function(w) {
      return w.frame().x < focused_window.frame().x
    })
    .value()

  // Phoenix.log("Windows left ", sorted_left.length)
  // sorted_left.forEach(w => Phoenix.log("\t" + w.hash() + " " + w.frame().x));

  const last = _.last(sorted_left)
  if (last) {
      focused_window.magnet_to(last)
  }
});
